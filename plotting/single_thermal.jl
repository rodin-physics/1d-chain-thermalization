...
data = load_object(
    "data/Thermal/Single_σ0[45]_σdot0[30]_MemInf_λ1_Φ1_μ1_d60_ΩT0.0_τ100.jld2",
)
...
data = load_object("data/Thermal/Single_σ0[45]_σdot0[30]_MemInf_λ1_Φ-1_μ1_d60_ΩT0.0_τ100.jld2")
δ = data.τs[2] - data.τs[1]
rr = reduce(hcat, [data.ρs[ii, :] .- ii * data.α for ii = 1:size(data.ρs)[1]])
mx = maximum(abs.(rr))
# mx = 18.0
