...
rep_files = ["Single_σ0[225]_σdot0[0]_MemInf_λ1_Φ1_μ1_d60_ΩT1.0e-5_τ100.jld2",
...
attr_files = ["Single_σ0[225]_σdot0[0]_MemInf_λ1_Φ-1_μ1_d60_ΩT1.0e-5_τ100.jld2",
...
function hopping_freqs(sol::SystemSolution)
    ...
    times = [(τs[indices[ii+1]] - τs[indices[ii]]) for ii in 1:(length(indices)-1)]
...
## Plotting
fig = Figure(resolution = (1200, 1600), font = "CMU Serif", fontsize = 36, figure_padding = 30)
ax1 = Axis(fig[1, 1], xlabel = L"\tau", ylabel = "Time between Hops")
ax2 = Axis(fig[2, 1], xlabel = L"\tau", ylabel = "Time between Hops")

for ind in 1:length(rep_files)
    (xs, ys) = hopping_freqs(rep_data[ind])
    scatter!(ax1,xs, ys, color = colors[ind], markersize = 8, label = L"\omega_T = %$(rep_data[ind].ωT)")
    hlines!(ax1, [mean(ys)], color = colors[ind], linewidth = 5)
end

for ind in 1:length(attr_files)
    (xs, ys) = hopping_freqs(attr_data[ind])
    scatter!(ax2, xs, ys, color = colors[ind], markersize = 8, label = L"\omega_T = %$(attr_data[ind].ωT)")
