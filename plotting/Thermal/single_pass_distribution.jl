...
σdot0 = 50.0
ωT = 0.0
colors = [my_blue, my_green, my_vermillion, my_red, my_yellow]
...
for ωT in ωTs 
    println(ωT)
    # x_range = range(1, 80, length = range_len) |> shuffle
    # x_range2 = range(0, 80, length = range_len) |> shuffle
    # mean_data = zeros(range_len)
    # var_data = zeros(range_len)
    # p = Progress(range_len)

    # Threads.@threads for ii in 1:range_len
    #     mean_data[ii] =  Δ_thermal_analytic(x_range[ii], Φ0, λ, ωmax, ωT)
    #     var_data[ii] =  Δ_thermal_variance(x_range2[ii], Φ0, λ, ωmax, ωT)
    #     next!(p)
    # end

    # indices = sortperm(x_range)
    # indices2 = sortperm(x_range2)
    # x_range = x_range[indices]
    # x_range2 = x_range2[indices2]
    # mean_data = mean_data[indices]
    # var_data = var_data[indices2]

    # writedlm("data/Thermal/MeanΔ_ωT$(ωT).dat", (x_range, mean_data))
    # writedlm("data/Thermal/VarΔ_ωT$(ωT).dat", (x_range2, var_data))

    mean_data = readdlm("data/Thermal/MeanΔ_ωT$(ωT).dat")
    var_data = readdlm("data/Thermal/VarΔ_ωT$(ωT).dat")

    lines!(ax1, mean_data[1,:], mean_data[2,:], color = colors[findfirst(x -> x == ωT, ωTs)], linewidth = 4, label = L"\omega_T = %$(ωT)")
    lines!(ax2, var_data[1,:], sqrt.(var_data[2,:]), color = colors[findfirst(x -> x == ωT, ωTs)], linewidth = 4)
...
lines!(ax1, x_range_nt, Δ_analytic.(x_range_nt, Φ0, λ, ωmax), color = my_black, linewidth = 3, label = "Non-Thermal")
...
# save("Loss_Mean_StdDev_test.pdf", fig)
fig
