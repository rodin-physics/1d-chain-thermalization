...
function particle_mean_displacement(data, box; periodic = false)
    σs_unfolded = traj_unfold(data, box, periodic = periodic)
    return (vec(mean(σs_unfolded, dims = 1)))
end
...
#     data = motion_solver(system, Φ0, λ, α, σ0, σdot0, μ, tTraj, 10, τ; threads = true, box = box, periodic = true, bias = bias_val)
#     save_object("data/Thermal/Bias/Bias$(bias_val)_speed0.0_ωT$(tTraj.ωT)_mem10.jld2", data)
# end
...
# for ωT in ωTs 
#     data = load_object("data/Thermal/Bias/Bias0.07_speed0.0_ωT$(ωT)_mem10.jld2")
...
#     #     lines!(ax1,  data.τs[2:end], speeds[particle, :], linewidth = 2, linestyle = :dash, color = colors[findfirst(x -> x == ωT, ωTs)], label = L"\omega_T = %$(ωT)")
#     # end
