...
## Plotting 
fig = Figure(resolution = (2000, 1000), fontsize = 36, figure_padding = 40)
...
ax3 = Axis(fig[1, 2], xticklabelsvisible = false, title = "Bias = 0.01")
ax4 = Axis(fig[2, 2], xlabel = L"\tau",)

xs = range(0, 1000, length = 1000) |> collect
D0 = 30
...
for ωT in ωTs 
    ...
    lines!(ax2, log.(times), log.(rmsd_vals), color = colors[findfirst(x -> x == ωT, ωTs)], linewidth = 4, label = L"\omega_T = %$(ωT)")

    ys = √(2 * √(2 * π * data.ωT) * α * exp(- data.Φ / effective_T(ωmax, data.ωT))) .* sqrt.(xs)
    lines!(ax2, log.(times[1:100:end]), log.(D0 * sqrt(effective_T(ωmax, data.ωT)) * exp(-data.Φ / effective_T(ωmax, data.ωT)) .* sqrt.(times[1:100:end])), linewidth = 3, linestyle = :dash, color = colors[findfirst(x -> x == ωT, ωTs)])
    # lines!(ax2, xs,ys, linewidth = 3, linestyle = :dash, color = colors[findfirst(x -> x == ωT, ωTs)])
end 

# Bias data
bias_val = 0.01 
for ωT in ωTs
    data = load_object("data/Thermal/Bias/Bias0.01_speed0.0_ωT$(ωT)_mem10_τ1000.jld2")
