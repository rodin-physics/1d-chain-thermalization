...
function get_particle_energy_dist(data, drop)
    ...
    for particle in 1:num_P
        ...
        pot_en = [sum((Φ0 .* exp.(-(data.ρs[:, t] .- data.σs[particle, t]) .^ 2 ./ (2 * λ^2))) ./ data.ωT) for t in eachindex(data.σs[particle, :])]
...
#     res = motion_solver(system, Φ0, λ, α, σ0, σdot0, μ, tTraj, Inf, τ; threads = true, box = box)

#     save_object("data/Thermal/Thermalization/Particle$(nParticles)_ωT$(tTraj.ωT)_τ$(τ)_memInf.jld2", res)
# end
...
for ωT in ωTs 
    hist_data = get_particle_energy_dist(load_object("data/Thermal/Thermalization/Particle25_ωT$(ωT)_τ500_mem10.jld2"), 100)
